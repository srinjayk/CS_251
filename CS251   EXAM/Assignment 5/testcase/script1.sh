...
touch result1.csv
echo "Line_number,execution_frequency" > result1.csv
echo "" >> result1.csv
awk '{print $2 " ," $1}' output | sed s/":.* "//g | sed s/":.*"//g >> result1.csv

############################# CODE FOR BRANCH BIAS ##################################

echo "" >> result1.csv
echo "Branch_line_number,bias" >> result1.csv
echo "" >> result1.csv
#gcov -b $C_file.gcno > output3
fgrep -n "branch" $C_file.c.gcov | awk '{print $1 " " $2 " " $4}' | sed s/:/' '/g | awk '$3==1{print $2 " " $3 ","$4/100}' >> result1.csv
